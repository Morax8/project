...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <form action="{{ route('jurusan.update', ['type' => 'tp', 'id' => $tpjurusan->id]) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="form-group">
                    <input type="hidden" class="form-control" name="id" value="{{ $tpjurusan->id }}">
                </div>
                <div class="form-group">
                    <label for="">title</label>
                    <input type="text" class="form-control" name="title" placeholder="title"
                        value="{{ $tpjurusan->title }}">
                </div>
                <div class="form-group">
                    <label for="">Content</label>
                    <textarea id="file-picker" class="form-control" rows="30" name="content"
                        placeholder="Content">{{ $tpjurusan->content }}</textarea>
                </div>
                <div class="form-group">
